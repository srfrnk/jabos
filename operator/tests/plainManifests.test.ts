...
test('plainManifests sync', async () => {
  const req = {
    body: {
      controller: {
        metadata: {
      ...
      object: {
        ...
        spec: {
          ...
          commitReplacementString: 'commitReplacementString_value',
          replacements: {}
        }
      },
      related: {
  ...
  await plainManifests.sync(req, res as unknown as Response, () => { /* NOOP */ });
  expect(res.json.mock.calls[0]).toMatchSnapshot();
  expect(res.status).toHaveBeenCalledWith(200);
});

test('plainManifests customize', async () => {
  const req = {
    body: {
      parent: {
        metadata: {
  ...
  const res = mockResponse();
  await plainManifests.customize(req, res as unknown as Response, () => { /* NOOP */ });
  expect(res.json.mock.calls[0]).toMatchSnapshot();
  expect(res.status).toHaveBeenCalledWith(200);
});

test('plainManifests finalize leave', async () => {
  const req = {
    body: {
      controller: {
        metadata: {
  ...
  const res = mockResponse();
  await plainManifests.finalize(req, res as unknown as Response, () => { /* NOOP */ });
  expect(res.json.mock.calls[0]).toMatchSnapshot();
  expect(res.status).toHaveBeenCalledWith(200);
});

test('plainManifests finalize delete', async () => {
  const req = {
    body: {
      controller: {
        metadata: {
